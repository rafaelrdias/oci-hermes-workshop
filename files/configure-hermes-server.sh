...
OCI_GENAI_PROJECT_ID="${OCI_GENAI_PROJECT_ID:-$(read_env OCI_GENAI_PROJECT_ID)}"
...
if [[ "$mode" == "--full" || "$mode" == "--oci-only" ]]; then
  if [[ ! "$OCI_GENAI_PROJECT_ID" =~ ^ocid1\.generativeaiproject\. ]]; then
    printf 'OCI Generative AI project OCID is missing or invalid. Re-run the GRU bootstrap.\n' >&2
    exit 1
  fi
...
project_id = os.environ["OCI_GENAI_PROJECT_ID"]
config = load_config()
