variable "region" {
  description = "OCI region for the Resource Manager stack, network, VM, Generative AI project and model. This alternative is fixed to Brazil East (Sao Paulo), region key GRU."
  type        = string
  default     = "sa-saopaulo-1"

  validation {
    condition     = var.region == "sa-saopaulo-1"
    error_message = "This alternative workshop stack must run in Brazil East (Sao Paulo): region must be sa-saopaulo-1 (GRU)."
  }
}
...
variable "genai_region" {
  description = "OCI Generative AI region. Fixed to Brazil East (Sao Paulo) so all regional workshop resources stay in GRU."
  type        = string
  default     = "sa-saopaulo-1"

  validation {
    condition     = var.genai_region == "sa-saopaulo-1"
    error_message = "OCI Generative AI must run in Brazil East (Sao Paulo): genai_region must be sa-saopaulo-1 (GRU)."
  }
}
