...
OCI_GENAI_REGION="${OCI_GENAI_REGION:-sa-saopaulo-1}"
OCI_GENAI_MODEL="${OCI_GENAI_MODEL:-openai.gpt-oss-120b}"
OCI_GENAI_BASE_URL="${OCI_GENAI_BASE_URL:-https://inference.generativeai.${OCI_GENAI_REGION}.oci.oraclecloud.com/openai/v1}"
OCI_GENAI_PROJECT_ID="${OCI_GENAI_PROJECT_ID:-}"
...
if [[ ! "$OCI_GENAI_PROJECT_ID" =~ ^ocid1\.generativeaiproject\. ]]; then
  printf 'OCI_GENAI_PROJECT_ID is empty or invalid.\n' >&2
  exit 1
fi
...
temp_env="$(mktemp "$HOME/.hermes/.env.XXXXXX")"
if [[ -f "$env_file" ]]; then
  awk 'index($0, "OCI_GENAI_PROJECT_ID=") != 1 { print }' "$env_file" > "$temp_env"
fi
printf 'OCI_GENAI_PROJECT_ID=%s\n' "$OCI_GENAI_PROJECT_ID" >> "$temp_env"
chmod 600 "$temp_env"
