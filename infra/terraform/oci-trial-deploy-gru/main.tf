locals {
  ...
  ssh_config_alias     = var.ssh_config_alias != "" ? var.ssh_config_alias : "hermes-oci-gru"
  hermes_base_url      = "https://inference.generativeai.${var.genai_region}.oci.oraclecloud.com/openai/v1"
  ...
  common_freeform_tags = merge(var.freeform_tags, { "workshop" = "tdc-hermes-oci", "region-key" = "GRU" })
}
...
resource "oci_generative_ai_project" "hermes" {
  compartment_id = var.compartment_ocid
  display_name   = "${var.prefix}-genai-project"
  description    = "OpenAI-compatible project for the Hermes Agent workshop in GRU"
...
resource "oci_core_instance" "hermes" {
  ...
  metadata = {
    ...
    user_data = base64encode(templatefile("${path.module}/cloud-init.yaml.tftpl", {
      ...
      genai_project_id      = oci_generative_ai_project.hermes.id
...
resource "oci_identity_policy" "hermes_genai" {
  ...
  name           = "${var.prefix}-genai-chat-gru"
  description    = "Allow OCI Generative AI API keys to call the Hermes workshop model in GRU"
  statements = [
