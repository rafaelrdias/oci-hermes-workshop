...
def test_inference(token, owner):
    ...
    while time.monotonic() < deadline:
        ...
        if result.returncode != 4:
            telegram(token, "sendMessage", {"chat_id": owner, "text":
                "Teste de ferramenta falhou; o agente não foi liberado. Peça ao atendente para consultar a documentação de diagnóstico."})
    ...
    if not notice.exists():
        telegram(token, "sendMessage", {"chat_id": owner, "text":
            ...
            "Continuarei tentando. Se persistir, consulte o atendente ou remova a Stack com Destroy para encerrar o consumo."})
