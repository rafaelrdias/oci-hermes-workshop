...
run "gru" {
  ...
  assert {
    ...
    error_message = "GRU deve manter chat restrito ao Llama, à região GRU e ao compartment da aplicação."
  }
  assert {
    condition     = oci_core_instance.hermes.shape_config[0].ocpus == 1 && oci_core_instance.hermes.shape_config[0].memory_in_gbs == 8
    error_message = "Tamanho padrão da VM alterado."
  }
}
...
run "ord_grok_43" {
  ...
  assert {
    ...
    error_message = "ORD deve autorizar todos os modelos de chat somente em Chicago e no compartment da aplicação."
  }
  assert {
