locals {
  ...
  tags                  = { purpose = "hermes-oci-eventos", managed_by = "terraform" }
...
resource "oci_identity_dynamic_group" "hermes" {
  ...
  compartment_id = var.tenancy_ocid
  name           = "${var.prefix}-vm"
  description    = "Identidade da única VM Hermes deste ambiente"
...
resource "oci_identity_policy" "hermes" {
  ...
  description    = "Somente Chat no compartment da aplicação e região selecionada; todos os modelos em ORD"
  statements = [
